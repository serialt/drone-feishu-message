...
func main() {
	...
	year := time.Now().Year()
	app.Copyright = fmt.Sprintf("© 2021-%d Serialt Simau", year)
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			...
			EnvVars: []string{"PLUGIN_TIPS_TITLE"},
		},
		&cli.StringFlag{
			Name:    "config.token,access_token,token",
			Usage:   "DingTalk webhook access token",
			EnvVars: []string{"PLUGIN_ACCESS_TOKEN", "PLUGIN_TOKEN"},
		},
		&cli.StringFlag{
		...
		&cli.StringFlag{
			...
			EnvVars: []string{"TPL_BUILD_STATUS_SUCCESS", "PLUGIN_TPL_BUILD_STATUS_SUCCESS"},
		},
		&cli.StringFlag{
			Name:    "tpl.build.status.failure",
			Usage:   "tpl.build status for replace failure",
			EnvVars: []string{"TPL_BUILD_STATUS_FAILURE", " PLUGIN_TPL_BUILD_STATUS_FAILURE"},
		},
		&cli.StringFlag{
			Name:    "custom.pic.url.success",
			Usage:   "custom success picture url",
			EnvVars: []string{"SUCCESS_PICTURE_URL", "PLUGIN_SUCCESS_PIC"},
		},
		&cli.StringFlag{
			Name:    "custom.pic.url.failure",
			Usage:   "custom failure picture url",
			EnvVars: []string{"FAILURE_PICTURE_URL", "PLUGIN_FAILURE_PIC"},
		},
		&cli.StringFlag{
			Name:    "custom.color.success",
			Usage:   "custom success color for title in markdown",
			EnvVars: []string{"SUCCESS_COLOR", "PLUGIN_SUCCESS_COLOR"},
		},
		&cli.StringFlag{
			Name:    "custom.color.failure",
			Usage:   "custom failure color for title in markdown",
			EnvVars: []string{"FAILURE_COLOR", "PLUGIN_FAILURE_COLOR"},
		},
		&cli.StringFlag{
			Name:    "custom.tpl,tpl",
			Usage:   "custom tpl",
			EnvVars: []string{"PLUGIN_TPL", "PLUGIN_CUSTOM_TPL"},
		},
		&cli.StringFlag{
			Name:    "tpl.repo.full.name",
			Usage:   "tpl custom repo full name",
			EnvVars: []string{"PLUGIN_TPL_REPO_FULL_NAME", "TPL_REPO_FULL_NAME"},
		},
		&cli.StringFlag{
			Name:    "tpl.repo.short.name",
			Usage:   "tpl custom repo short name",
			EnvVars: []string{"PLUGIN_TPL_REPO_SHORT_NAME", "TPL_REPO_SHORT_NAME"},
		},
		&cli.StringFlag{
			Name:    "tpl.commit.branch.name",
			Usage:   "tpl custom commit branch name",
			EnvVars: []string{"PLUGIN_TPL_COMMIT_BRANCH_NAME", "TPL_COMMIT_BRANCH_NAME"},
		},
		&cli.StringFlag{
